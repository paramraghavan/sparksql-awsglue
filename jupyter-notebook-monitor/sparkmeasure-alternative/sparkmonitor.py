...
def _get(path, port):
    try:
        with urllib.request.urlopen(
                f"http://localhost:{port}{path}", timeout=5) as r:
...
def _app_id(port):
    apps = _get("/api/v1/applications", port)
    return apps[0]["id"] if apps else None


def _stage_ids_done(port, app_id):
    stages = _get(f"/api/v1/applications/{app_id}/stages", port) or []
    return {s["stageId"] for s in stages if s.get("status") == "COMPLETE"}


def _new_stages(port, app_id, before):
    stages = _get(f"/api/v1/applications/{app_id}/stages", port) or []
...
def _render_report(stages, elapsed_s):
...
_PORT   = _find_spark_port()
_APP_ID = _app_id(_PORT) if _PORT else None
...
def measure(line, cell):
    ...
    if not _PORT or not _APP_ID:
        ...
        <div style="font-family:Arial,sans-serif;max-width:560px;padding:12px 16px;
          ...
          The Spark UI runs on port 4040 — sparkmonitor checks ports 4040 to 4045.<br><br>
          If you just started the kernel, re-run Cell 1 after the SparkSession is ready.
          </span>
        </div>"""))
        exec(cell, globals())
        return

    before  = _stage_ids_done(_PORT, _APP_ID)
    t0      = time.monotonic()
    exec(cell, globals())
    elapsed = time.monotonic() - t0
    time.sleep(0.6)   # allow last stage status to propagate
    new     = _new_stages(_PORT, _APP_ID, before)
    _render_report(new, elapsed)
...
if _PORT and _APP_ID:
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:6px 0">
      ...
      <div style="border:1px solid #ccc;border-top:none;border-radius:0 0 8px 8px;
        ...
        <strong>Connected</strong> to Spark on port {_PORT}.<br><br>
...
df.filter(df.year == 2024).groupBy("region").count().show()</pre>
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:6px 0;
      ...
      <strong>sparkmonitor loaded, but Spark is still starting up.</strong><br>
      <span style="font-size:12px;line-height:1.7">
      Re-run this cell once your SparkSession is ready,
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:6px 0;
                padding:12px 16px;background:#FFF8E6;border-left:4px solid #E8A000;
                border-radius:0 8px 8px 0;font-size:13px;color:#7A4900">
      <strong>sparkmonitor loaded, but no Spark UI found on ports 4040-4045.</strong><br>
      <span style="font-size:12px;line-height:1.7">
      Start a SparkSession and then re-run this cell.
      </span>
    </div>"""))
