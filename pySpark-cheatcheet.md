...
# Right join
df1.join(df2, "id", "right")  # Shorthand when column names match
...
# Left semi join (rows in df1 that ARE in df2, no df2 columns)
...
```python
# Case conversion
df.select(F.upper("name"), F.lower("name"), F.initcap("name"))
...
df.select(
    F.substring("name", 1, 3),  # First 3 characters
    F.length("name"),
...
# Split and explode
df.select(F.split("tags", ",").alias("tag_array"))
df.select(F.explode(F.split("tags", ",")).alias("tag"))
...
```python
# Current date/time
df.select(F.current_date(), F.current_timestamp())
...
df.select(
    ...
    F.dayofweek("date_col"),   # 1=Sunday, 7=Saturday
    F.weekofyear("date_col"),
    F.quarter("date_col"),
...
# Format dates
df.select(F.date_format("date_col", "MMM dd, yyyy"))
```
...
pipeline = Pipeline(stages=[
    StringIndexer(inputCol="category", outputCol="cat_idx"),
    OneHotEncoder(inputCol="cat_idx", outputCol="cat_vec"),
    VectorAssembler(
        inputCols=["age", "salary", "cat_vec"],
        outputCol="features"
    ),
